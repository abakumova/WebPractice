function showUsers(sortBy) {
    ...
    if (sortBy == undefined || typeof sortBy != "string") {
        url = "../laboratorna_2/controllers/showAllUsersController.php";
    } else {
        url = "../laboratorna_2/controllers/showAllUsersController.php/?sortBy" + sortBy + "=true";
    }
...
function formatUsersTable(output, users) {
    ...
    for (const user in users) {
        let role = users[user].title == "admin" ? "Admin" : "User";
        let id = users[user].id;
        output +=
            "<tbody><tr>" +
            '<td><a href="../laboratorna_2/pages/profile.php?id=' +
            id +
            '">' +
            id +
            "</a></td>" +
